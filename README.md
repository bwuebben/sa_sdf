...
This runs the comprehensive training script with mock data:
- **Data**: 120 months of synthetic data, 50 assets, 132 characteristics
- **Training**: 5 epochs with progress bars and real-time metrics
- **Output**: Timestamped directory with visualizations and detailed report

**Example Output:**
```
output/
  └── run_2026-01-02_14-30-45/
      ├── report.md                      # Comprehensive training report
      ├── cumulative_returns.png         # Portfolio performance
      ├── drawdown.png                   # Drawdown analysis
      ├── loss_decomposition.png         # Loss components (4 panels)
      ├── performance_metrics.png        # Sharpe, vol, drawdown evolution
      ├── turnover.png                   # Monthly turnover vs benchmarks
      ├── portfolio_composition.png      # Long/short positions
      ├── cvar_evolution.png             # CVaR threshold learning
      └── weights_heatmap.png            # Portfolio weights over time
```

**Training Metrics Displayed:**
- Loss decomposition (pricing error, transaction costs, CVaR)
- Portfolio performance (Sharpe ratio, returns, volatility, max drawdown)
- Transaction costs and turnover
- Portfolio composition (long/short positions, concentration)
- Comparison to paper benchmarks (SA-SDF vs KKMU Transformer)
...
The training script automatically generates comprehensive visualizations and reports for each run:

### Generated Charts

1. **Cumulative Returns** (`cumulative_returns.png`)
   - Portfolio growth over time with filled area chart
   - Monthly performance tracking

2. **Drawdown Analysis** (`drawdown.png`)
   - Underwater periods visualization
   - Maximum drawdown identification

3. **Loss Decomposition** (`loss_decomposition.png`)
   - 4-panel view: Total Loss, Pricing Error, Transaction Costs, CVaR
   - Evolution across training epochs

4. **Performance Metrics** (`performance_metrics.png`)
   - Sharpe Ratio evolution (vs paper target: 1.43, KKMU: 1.15)
   - Volatility trends
   - Max Drawdown evolution (vs paper target: 26.8%, KKMU: 33.4%)

5. **Turnover Analysis** (`turnover.png`)
   - Monthly portfolio turnover
   - Comparison to benchmarks (paper: 38.6%, KKMU: 71.2%)

6. **Portfolio Composition** (`portfolio_composition.png`)
   - Long vs short position counts
   - Net exposure over time

7. **CVaR Evolution** (`cvar_evolution.png`)
   - Learning trajectory of CVaR threshold (ν)
   - Risk parameter optimization

8. **Weights Heatmap** (`weights_heatmap.png`)
   - Portfolio allocation patterns over time
   - First 20 assets visualization

### Training Report

Each run generates a `report.md` containing:
- **Hyperparameters**: Learning rate, epochs, model configuration
- **Final Performance**: Sharpe ratio, returns, volatility, drawdown, turnover
- **Benchmark Comparison**: Side-by-side comparison with paper results
- **Training Progress**: Epoch-by-epoch metrics table
- **Key Findings**: Automated analysis vs paper targets
...
cd output/run_*  # Use tab completion for timestamp
...
2. **Run training with visualizations:**
   ```bash
   python train.py
   ```

3. **Review results:**
   - Check console output for training progress and metrics
   - Navigate to `output/run_YYYY-MM-DD_HH-MM-SS/` for charts and report
   - Compare your results to paper benchmarks in `report.md`

---

**Note**: This implementation uses mock data for demonstration. Real-world application requires:
- Historical stock characteristics (132 dimensions from Jensen et al. 2023)
...
The training script generates comprehensive visualizations and performance reports automatically, making it easy to analyze model behavior and compare against published benchmarks.
